...
export class Migration20241010104821 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table "company" ("id" varchar(255) not null, "name" varchar(255) not null, "description" varchar(255) not null, "email" varchar(255) not null, "phone_number" varchar(255) not null, "website" varchar(255) not null, constraint "company_pkey" primary key ("id"));`);

    this.addSql(`create table "building" ("id" varchar(255) not null, "name" varchar(255) not null, "address" jsonb not null, "company_id" varchar(255) not null, constraint "building_pkey" primary key ("id"));`);
    this.addSql(`alter table "building" add constraint "building_company_id_unique" unique ("company_id");`);
    ...
    this.addSql(`create table "user" ("id" varchar(255) not null, "full_name" varchar(255) not null, "email" varchar(255) not null, "password" varchar(255) not null, "type" text check ("type" in ('b2b', 'b2c')) not null, "company_id" varchar(255) null, constraint "user_pkey" primary key ("id"));`);
    this.addSql(`alter table "user" add constraint "user_company_id_unique" unique ("company_id");`);
