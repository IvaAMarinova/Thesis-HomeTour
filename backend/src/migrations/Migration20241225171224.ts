...
export class Migration20241225171224 extends Migration {
  ...
  override async down(): Promise<void> {
    this.addSql(
      `create table "building" ("id" uuid not null, "name" varchar(255) not null, "description" text not null, "address" jsonb not null, "company_id" uuid null, constraint "building_pkey" primary key ("id"));`,
    );
    ...
    this.addSql(
      `alter table "property_entity" add column "building_id" uuid null;`,
    );
    this.addSql(
